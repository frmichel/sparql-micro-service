#!/bin/sh
# This script customizes the configuration files (config.ini) and SPARQL file (insert.sparql, construct.sparql)
# of each SPARQL micro-serive by replacing the "example.org" hostname and "<api_key>" values commited on the public repo.
#
...
# The machine where the services are deployed. Will replace the 'example.org'
