...
The SD graph in file ```ServiceDescription.ttl``` is described in article [4](../README.md#Publications). Very succinctly, it describes an ```sd:Service``` instance whose data source (```dct:source```) is a Web API (```schema:WebAPI```) that has an search action (```schema:potentialAction```).

The service parameters are translated into elements of the SD graph as follows:
 
Parameter | Mandatory/Optional | Description
------------ | ------------- | -------------
Input arguments | Mandatory | Set of ```hydra:IriTemplateMapping``` resources (```hydra:mapping```) associated with the Web API potential action.
Web API query string | Mandatory | A ```hydra:IriTemplate``` (```hydra:template```) providing the Web API query string template. It contains placeholders for the input arguments.
